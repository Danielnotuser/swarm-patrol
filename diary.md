...
```bash
   ...
   source /Swarm-SLAM/install/setup.bash; \
   export ROS_DOMAIN_ID=0 &&\
   ros2 run tf2_tools view_frames --ros-args -r /tf:=/r0/tf -r /tf_static:=/r0/tf_static"
```
```bash
    docker cp swarmslam:frames_2026-05-10_17.40.37.pdf  ~/Swarm-SLAM/data/
```
```bash
    docker exec -it swarmslam bash -c "\
...
в начале недели нав поехал с помощью RPP плагина и правильных фреймов в путях, но съезжал с путей сразу и ехал не понятно как, пытаясь подловить позы с разных прямых маршрута

нав плохо работал из-за слишком плотных путей дарпа - перскакивал по ним, поэтому работа ведется по расплотнению дарпа

я пытался: еще больше мельчить сетку в STC (она сейчас увеличивает сетку в два раза), но внутренние формулы не поддались адаптации; 
дальше пробовал убрать из параллельных линий средние, но идея провалилась по причине, что линии разных размеров и какая-то из них может вести в другую часть карты;
также пытался навигацию по-другому делать: медленнее ехать, поменять плагин на строгое следование по позам (но не нашел);
были еще какие-то попытки, но итог такой:
я уменьшаю еще до разделения зон дарпом сетку в два раза и STC получается увеличивает ее в два раза и получается изначальная сетка с полным покрытием, однако в два раза менее плотным

этот способ в итоге тоже не подошел, так как он закрывает проходы, через которые потенциально можно проехать

пока остановился на способе просто уменьшить кол-во точек на прямых, который я до этого пытался использовать, но в прошлый раз там только две точки на прямой оставались, сейчас каждую 2 удаляю


нода для переделки таймстемпов поинтклауда для локал костмап нав2

# 13.05.26

3 дня пытался понять, почему не постятся локал костмапы, оказалось в нав2 есть прелестная фича, нужно убрать параметр use_sim_time вообще, чтобы оно заработало, мне, к сожалению, не хватило смекалки сразу просечь фишку

в итоге сделал ноду переделки таймстемпов поинтклауда, через множество попыток вывел рабочую на данный момент схему:

поинтклауд новый просто заменяет на свой фрейм r0/laser_frame и заменяет на now() time, 

также сделан трансформ переброска r0/odom->r0/base_link из /r0/tf в robot0_map->/r0/base_link в /tf

возможно система работает и с base_link = robot0_current_pose, как я раньше делал, но до этого была проблема, что поинтклауд крутился вместе с роботом (двигались стены как будто)

в целом навигация работает сейчас в связке r0/laser_frame - robot0_map с добавленным плагином убирания шумов и voxel_layer убирает неподтвержденные 2 раза точки (из-за поворота появляются стены на пути)

надо сделать: 
* навигацию подредактировать для неск роботов (айди добавить)
* сделать еще наверное 1 карту, для каждой карты подобрать параметры навигации
* сделать аномалии с маркерами
* протестировать на 3 картах с 3-5 расстояниями аномалии до робота (скорость обнаружения в зависимости от площади зоны и расстояния до аномалии)
* добавить отказ робота - для этого нужно отредактировать навигацию, чтобы при получении нового пути (отличающегося) он заново начинал, хотя наверно лучше просто посылать сигнал стоп работа
* протестировать отказ

также:
* добавить фронтир навигацию с сигналов wake_up дарпу
* добавить перезапуск фронтира (вместе со всеми слэм нодами) при аномалии 
* запуск дарпа с брокера: в сервисе wake_up айди брокера - брокер делает, остальные ждут, если слишком долго ждут номер брокера + 1 % num и так далее

# 14.05.26

пофиксил obstacle_dilation у дарпа

также сделать: в darp_path_follower поменять везде self.frame_id на robotX_map

сделано, навигация работает для двух роботов (правда почему-то локализация иногда ломается)

сделан набросок anomaly_detection, для него также сделана доп публикация от дапа /darp/full_area, чтобы просто иметь карту с препятствиями, не думая о зонах, 
единственное не понятно, нужно ли постить каждому роботу в своем мап фрейме или оставить один с робот0_мап фреймом

также скорее всего нужно использовать поинтклауд_рил от нава, потому что фрейм шалалайский от оригинального поинтклауда

# 15.05.26

удивительно получилось сделать ноду детекции аномалий с исчезновением и появлением препятствий

для него в итоге /rX/darp/full_area постится для каждого робота для совместимости с фреймами и поинтклауд_рил использую

добавил в навигацию остановку при обнаружении аномалии, вообще нужно сделать так, чтобы cslam перезапускался, но пока не знаю, буду ли

добавил хартбит чекер для этого: добавил в дарп параметр айдишников роботов, в нав добавил слушатель и остановку по сбою

временно(?) меняю сервис /darp/wake_up на топик, так как зено не протаскивает сервисы, по всей видимости

проверил включение дарпа и мену навигации

# 17.05.26

при обнаружении аномалии - фронтиры у робота, обнаружившего аномалию, - другие роботы штатно навигируют - 
робот просмотрел все около аномалии, запускает дарп - роботы находят ближайшую точку на новом пути и начинают навигацию

f1 меры

# 23.05.26

где-то дня 4 делал фронтиры, сейчас они в целом норм работают, но пока только 1 раз проверил реакцию на аномалию

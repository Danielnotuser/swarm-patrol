...
<p align="center">
  <em>Мульти-роботная система, объединяющая Collaborative SLAM, исследование на основе фронтьеров, DARP-разделение территории, детекцию аномалий и автономную навигацию Nav2.</em>
</p>
...
- **Мульти-роботная координация**: Протокол резервирования предотвращает выбор одной и той же области разными роботами.
- **Адаптивное исследование**: Переключение между исследованием фронтьеров и следованием маршрутам DARP.
...
- **Робот-независимость**: Поддержка симуляции (Gazebo/Ignition) и реальных роботов (Ouster, Intel RealSense, OAK-D).
...
### `frontier_exploration` — Автономное исследование на основе фронтьеров
...
3. **Поиск фронтьеров** → FREE клетки, граничащие с UNKNOWN
...
6. **Координация** → Протокол резервирования: `exclusion_radius=3.0 м` предотвращает пересечение целей; штрафные оценки смещают роботов к незанятым фронтьерам
...
Когда все фронтьеры исчерпаны, система уведомляет DARP bridge о запуске покрытия территории.
...
2. Уменьшает разрешение карты фронтьеров до разрешения DARP (0.5 м) через `cv2.INTER_AREA`
...
- **BT Navigator:** Следование маршрутам DARP (`follow_path.xml`) или навигация к целям фронтьеров (`navigate_to_pose.xml`)
...
- Нода исследования реагирует:
  ...
  3. Публикует `anomaly_cleared`, когда все фронтьеры исчерпаны
...
│   │   │   ├── frontier_finder.py      # Поиск и кластеризация фронтьеров
